...
def test_per_bot_flag_enables_test_bot_without_waking_others():
    ...
    asyncio.run(flags.set_flag("bots_enabled:frunze_tours_tg", True))   # включаем точечно тест-бот

    tours_bot = Orchestrator(channel=None, bot=BotConfig(id="frunze_tours_tg", scenario="tours"))
    whatsapp_bot = Orchestrator(channel=None, bot=BotConfig(id="wa_main", scenario="visa"))
    dev_bot = Orchestrator(channel=None)  # без bot → только глобальный флаг

    assert asyncio.run(tours_bot._bots_on()) is True       # персональный флаг переопределил
